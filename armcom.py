	...
	def DoEnemyAdvance(self):
		...
		if not campaign.day_map.player_node.exit:
			for node in campaign.day_map.nodes:
				if node.exit:
					if len(GetPath(campaign.day_map.player_node, node)) > 0:
						return
					break
...
def UnitInfo(mx, my):
	...
	# TEMP - disabled
	# if no unit here, show info about the hex
	map_hex = Screen2Hex(mx, my)
	if map_hex is not None:
		if map_hex.smoke_factors > 0:
			text = str(map_hex.smoke_factors)
		else:
			text = 'No'
		text += ' active smoke factors in this hex'
		Message(text)
